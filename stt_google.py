...
def listen_for_speech(threshold=THRESHOLD, num_phrases=-1, output = None):
    """
    ...
    while (num_phrases == -1 or n > 0):
        ...
        if(sum([x > THRESHOLD for x in slid_win]) > 0):
            ...
            filename = save_speech(list(prev_audio) + audio2send, p, output)
            if(output != None):
                break
...
def save_speech(data, p, output = None):
    """ Saves mic data to temporary WAV file. Returns filename of saved 
        file """
    if(output == None):
        filename = 'output_'+str(int(time.time()))
    else:
        filename = output.replace('.wav', '')
